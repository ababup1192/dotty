...
type alias ParsedValue = List Point


unparse : ParsedValue -> Result String String
unparse = unparseAst << toAst


toAst : ParsedValue -> Ast
toAst points =
  let
    toNDot (x, y) = NPoint x y
  in
...
list ast =
  case ast of
    NList npoints ->
      let
        concat acc p = Result.map (\acc_ -> p ++ acc_) acc
        points = List.foldl (\p acc -> Result.andThen (concat acc) p) (Ok "") <| List.map point npoints
      in
        Result.map (\points_ -> "[" ++ points_ ++ "]") points

    _ -> Err "Here must be NList."
